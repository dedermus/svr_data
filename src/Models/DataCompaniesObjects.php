...
class DataCompaniesObjects extends Model
{
    ...
    /**
     * Получаем список поднадзорных объектов компании
     *
     * @param array|int|false $company_ids
     * @param array|int|false $company_object_ids
     *
     * @return array
     */
    public static function companyObjectsList(
        array|int|false $company_ids = false, array|int|false $company_object_ids = false
    ): array {
        $query = DB::table(DataCompaniesObjects::getTableName());
        ...
        if ($company_ids !== false) {
            ...
            if (is_array($company_object_ids)) {
                $query->whereIn('company_object_id', $company_object_ids);
            } else {
                $query->where('company_object_id', $company_object_ids);
            }
        }

        return $query->orderBy('company_object_is_favorite', 'DESC')
            ->get()->toArray();
    }
